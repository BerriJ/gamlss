...
## ----------------------------------------------------------------------------------------
.gamlss.sm.list <- c(
  "cs", "scs", # smoothing cubic splines  "s" vc"
  ...
  RS <- function(n.cyc = control$n.cyc, no.warn = TRUE) {
    ...
    glim.fit <- function(f, X, y, w, fv, os, step = 1, control = glim.control(),
      ...
      while (abs(olddv - dv) > cc && itn < cyc) # MS Wednesday, June 26, 2002
      {
        ...
        dr <- f$dr(eta) # pmax(exp(eta), .Machine$double.eps)
        dr <- 1 / dr
        ## method 2
        dldp <- f$dldp(fv) # ((y - mu)^2 - sigma^2)/(sigma^3)
        d2ldp2 <- f$d2ldp2(fv) # -(2/(sigma^2))
        d2ldp2 <- ifelse(d2ldp2 < -1e-15, d2ldp2, -1e-15) # added 26-10-07
        wt <- -(d2ldp2 / (dr * dr))
        wt <- ifelse(wt > 1e+10, 1e+10, wt) # Mikis 9-10-14
        wt <- ifelse(wt < 1e-10, 1e-10, wt)
...
gamlss.control <- function(c.crit = 0.001, n.cyc = 20, mu.step = 1, sigma.step = 1,
  ...
  list(
    c.crit = c.crit, n.cyc = n.cyc, mu.step = mu.step, sigma.step = sigma.step, nu.step = nu.step,
...
glim.control <- function(cc = 0.001, cyc = 50, glm.trace = FALSE, bf.cyc = 30, #  step = 1,
  ...
  #                step <- 1}
  list(
    cc = cc, cyc = cyc, glm.trace = as.logical(glm.trace)[1], bf.cyc = bf.cyc, # step = step,
    bf.tol = bf.tol, bf.trace = as.logical(bf.trace)[1]
  ) # MS Thursday, January 8, 2004 at 17:53
}
## ----------------------------------------------------------------------------------------

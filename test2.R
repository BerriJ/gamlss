...
dr <- pmax(exp(eta), .Machine$double.eps) # f$dr(eta)
...
wv <- (eta - os) + (((y - mu)^2 - fv^2) / (fv^3)) / (dr * wt) # eta
...
while (abs(olddv - dv) > cc && itn < cyc) # MS Wednesday, June 26, 2002
{
    ...
    dr <- 1 / pmax(exp(eta), .Machine$double.eps) # f$dr(eta) #
    ## method 2
    ...
    wt <- -(d2ldp2 / (dr^2))
    wt <- pmax(pmin(wt, 1e+10), 1e-10)
    wv <- (eta - os) + (((y - mu)^2 - fv^2) / (fv^3)) / (dr * wt)
    # if (family$type == "Mixed") wv <- ifelse(is.nan(wv), 0, wv) ## TEST
    #   olddv <- dv
    #      dv <- sum(w*di)
    print(itn)
